...
Keep unsupported or ambiguous requirements intact, do not guess a gas or product. Do not invent facts.`;
  ...
  return {
    async select(history, message, signal, usage) {
      ...
      const stream = await sdk.chat.completions.create({ model: config.model, messages, tools: [SEARCH_TOOL],
    ...
    async explain(selection, result, signal, emit, usage) {
      const evidence = {
      ...
      const stream = await sdk.chat.completions.create({ model: config.model, stream: true,
